...
const WorldNews = () => {
    ...
    useEffect(() => {
        axios.get(`${API}/onlycat?limit=4&cat=62dc05bb8e9c47f21a1499b7`)
  ...
  return (
    <div className='col-md-4'>
        <div className={`${styles.hideinmobile}`}>
          <div className='mt-4'>
            <Link href='/category/বিশ্বজুড়ে'><p className={styles.america} style={{cursor: "pointer"}}>বিশ্বজুড়ে <BsChevronDoubleRight size="17px"/></p></Link>
            <div className={styles.hr2}></div>
          </div>
          <div className={styles.rajnitisec}>
            <div className='col-md-12 mt-2 border'>
              {/* <img src='img1.jpg' className='w-100'/> */}
              <Image src={`${IMG_API}/${worldnews[0]?.photo}`} width = {100} height = {60} layout="responsive"/>
              <div>
                <h3 className='p-2 pb-0 mb-0'>
                  <Link href={`/${worldnews[0]?.slug}`}>
                    <a className="pt-1 fw-bold fs-23">{worldnews[0]?.title}</a>
                  </Link>
                </h3>
              </div>
              <div className={styles.rajnitisecimg}>
                {worldnews && worldnews?.slice(1, 4).map((data, index) => 
        ...
        <div className={`${styles.hideindesktop}`}>
          <div className='mt-4'>
              <Link href='/categories/বিশ্বজুড়ে'><p className={styles.america} style={{cursor: "pointer"}}>বিশ্বজুড়ে <BsChevronDoubleRight size="17px"/></p></Link>
              <div className={styles.hr2}></div>
          </div>
                  ...
                  <div>
                    <h3 className='mt-2 p-2 py-0'>
                      <Link href={`/blogs/${worldnews[0]?.slug}`}>
                        <a>  
                          <p style={{fontSize: "23px", fontWeight: "bold"}} className="pt-1">{worldnews[0]?.title}</p>
                        </a>
                      </Link>
                    </h3>
                  </div>

                  {worldnews && worldnews?.map((data, index) => 

...
const AdsCreate = ({ router }) => {
    ...
    const deletemsg = () => toast.error("Ads Deleted!", {
        position: toast.POSITION.TOP_RIGHT
    });
    ...
    const { error, id, sizeError, saving, formData, title, link, alt, position, status, view, photo,  hidePublishButton } = values;
        ...
        createAds(formData, token).then(data => {
            if (data?.error) {
               setValues({ ...values, error: data.error });
                ...
                setValues({ ...values, title: '', saving:'', link: '', alt: '', position: '', view: '', status: '', photo: '', error: '' });
                setImage('');
                getPosts();
                setMsg(true);
            }
                                        ...
                                        <form onSubmit={publishAds}>
                                            <div className="form-group">
                                                ...
                                                <select value={position} onChange={handleChange('position')} class="form-select" aria-label="Default select example" required>
                                                    ...
                                                    <option value="topbanner">Top Ads (700 * 90)</option>
                                                    <option value="topbigbanner">Top Big Banner Ads (1250 * 156)</option>
                                                ...
                                                <tbody>
                                                    {items && items?.map((blog, i) =>
                                                        <tr key={i}>
                                                            ...
                                                            <td><img src={`${API}/ads/photo/${blog?.slug}`} width="100px" height="50px"/></td>

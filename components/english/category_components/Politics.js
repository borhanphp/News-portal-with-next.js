...
const Politics = () => {
    ...
    const [epolitics, setPolitics] = useState([]);

    useEffect(() => {
        axios.get(`${API}/eonlycat?cat=62e2dd5e4e2450054812c670`)
  ...
  return (
      <div className='col-md-4'>
        <div className={`${styles.hideinmobile}`}>
          <div className='mt-4'>
          <Link href='/en/ecategories/olitics'><p className={styles.america} style={{cursor: "pointer"}}>Politics <BsChevronDoubleRight size="17px"/></p></Link>
            <div className={styles.hr2}></div>
          </div>
          <div className={styles.rajnitisec}>
            <div className='col-md-12 mt-2 border'>
              {/* <img src='img1.jpg' className='w-100'/> */}
              <Image src={`${IMG_API}/${epolitics[0]?.photo}`} width = {100} height = {60} layout="responsive"/>
              <div>
                <h3 className='p-2 pb-0 mb-0'>
                  <Link href={`/en/eblogs/${epolitics[0]?.slug}`}>
                    <a className="pt-1 fw-bold fs-23">{epolitics[0]?.title}</a>
                  </Link>
                </h3>
              </div>
              <div className={styles.rajnitisecimg}>
                {epolitics && epolitics.slice(0, 4)?.map((data) => 
            ...
            <div className='col-12 px-0 mt-2 border'>
              ...
              <div>
                <h3 className='mt-2 p-2 py-0'>
                  <Link href={`/en/eblogs/${epolitics[0]?.slug}`}>
                    <a>  
                      <p style={{fontSize: "23px", fontWeight: "bold"}} className="pt-1">{epolitics[0]?.title}</p>
                    </a>
                  </Link>
                </h3>
              </div>

...
const Category = ({ category, subcategory, blogs, query, limit, page, count }) => {
    ...
    const fetchPosts = async (currentPage) => {
        const res = await fetch(`${API}/ecategory/${query.slug}?limit=${limit}&page=${currentPage}`);
                                ...
                                <div className='mb-3 p-2 pb-1' >
                                    ...
                                    {subcategory && subcategory.map((s, i) => 
                                        <Link key={i} href={`/subcategory/${s.slug}`}>
                                            <span>
                                                <span>&nbsp;&nbsp;</span>
                                                <button className='btn' style={{transition: "all 0.3s ease-in-out", borderBottom: "2px solid #02006d",backgroundColor: "#2980b9", color: "#fff", borderRadius: "4px"}}>{s.name}</button>
                                            </span>
                                        </Link>
